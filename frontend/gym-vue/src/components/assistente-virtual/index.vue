    ...
    <div :class="{ 'chat-container': true, open: chatBotOpen, closed: chatWasOpen }">
        ...
        <div class="chat-main">
            <div id="chat-message">
                ...
                <div v-for="(response, index) in userRespostas" :key="'response-' + index"
...
export default {
    ...
    methods: {
        ...
        teste(){
            if (!this.assitenteMenssage) {
                this.assitenteMenssage = true
                this.messageLoader = true

                setTimeout(() => {
                    for (let i = 0; i <= 2; i++) {
                        setTimeout(() => {
                            this.userRespostas.push({ message: this.assistenteScript[i].message, type: 'assistente', from: 'assistente-resposta' });
                        }, i * 3000);
                    }
    ...
    mounted() {
        ...
        console.log(this.assistenteScript);
    }
}
</script>
...
.resposta-massage {
    ...
    animation: popAnimation .5s forwards;
}
