  ...
  <section class="categorie-filter">
    ...
    <template v-if="productCategorie.length">
      <section class="categorie-product-container">
        <div class="categorie-top">
          <div>
            Exibindo <span>{{ itensPorPagina }} de {{ productCategorie.length }} produtos</span> nessa categoria
          </div>
          <div>
            Itens por Página:
    ...
    <template v-else>
      <section class="categorie-product-container categorie-not-found">
        <span>
          <h1>A categoria "<span>{{ categories }}</span>" infelizmente não foi encontrada :(</h1>
